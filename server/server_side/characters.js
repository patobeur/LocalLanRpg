const characters = {
	names: [
		"Moumba",
		"Wiko",
		"Squazzzza",
		"Gromp",
		"Zephyr",
		"Ignis",
		"Flora",
		"Shadow",
	],
	types: [
		"tank",
		"dps",
		"support",
		"tank",
		"speedster",
		"mage",
		"healer",
		"assassin",
	],
	xpNeededPerLv: [
		0, 1000, 3000, 6000, 10000, 15000, 21000, 28000, 36000, 45000, 55000,
		66000, 78000, 91000, 105000, 120000, 136000, 153000,
	],
	xpRewardedPerLv: [
		100, 400, 900, 1600, 2500, 3600, 4900, 6400, 8100, 10000, 12100, 14400,
		16900, 19600, 22500, 25600, 28900, 32400,
	],
	chars: {
		Moumba: {
			name: "Moumba",
			type: "tank",
			speed: 1,
			hitDistance: 5,
			autoAttackDamage: [40, 50, 70, 80, 90], // [skill lv1, lv2, lv3, lv4, lv5 ]
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [200, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 150,
			mana: 80,
			HealthRegeneration: 1.5,
			manaRegeneration: 1,
			physiqueArmor: 2,
			magicArmor: 1.5,
			scale: 1,
			svg: "moumba.svg",
			png: "Moumba.jpg",
			glb: "Moumba.glb",
		},
		Wiko: {
			name: "Wiko",
			type: "dps",
			speed: 1.1,
			hitDistance: 10,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [1, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 90,
			mana: 110,
			HealthRegeneration: 1.1,
			manaRegeneration: 1,
			physiqueArmor: 1.2,
			magicArmor: 1.2,
			scale: 1,
			svg: "wiko.svg",
			png: "Wiko.jpg",
			glb: "Wiko.glb",
		},
		Squazzzza: {
			name: "Squazzzza",
			type: "support",
			speed: 1,
			hitDistance: 10,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 110,
			mana: 130,
			HealthRegeneration: 1.1,
			manaRegeneration: 1,
			physiqueArmor: 1.5,
			magicAttack: 2,
			scale: 1,
			svg: "squazzzza.svg",
			png: "Squazzzza.jpg",
			glb: "Squazzzza.glb",
		},
		Gromp: {
			name: "Gromp",
			type: "tank",
			speed: 0.8,
			hitDistance: 6,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 200,
			mana: 50,
			HealthRegeneration: 2.0,
			manaRegeneration: 0.5,
			physiqueArmor: 3,
			magicArmor: 1,
			scale: 1,
			svg: "gromp.svg",
			png: "Gromp.jpg",
			glb: "Gromp.glb",
		},
		Zephyr: {
			name: "Zephyr",
			type: "speedster",
			speed: 1.5,
			hitDistance: 10,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 80,
			mana: 100,
			HealthRegeneration: 1.0,
			manaRegeneration: 1.5,
			physiqueArmor: 0.8,
			magicArmor: 0.8,
			scale: 1,
			svg: "zephyr.svg",
			png: "Zephyr.jpg",
			glb: "Zephyr.glb",
		},
		Ignis: {
			name: "Ignis",
			type: "mage",
			speed: 1.0,
			hitDistance: 10,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 90,
			mana: 200,
			HealthRegeneration: 0.8,
			manaRegeneration: 2.5,
			physiqueArmor: 0.5,
			magicArmor: 2,
			scale: 1,
			svg: "ignis.svg",
			png: "Ignis.jpg",
			glb: "Ignis.glb",
		},
		Flora: {
			name: "Flora",
			type: "healer",
			speed: 1.1,
			hitDistance: 8,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 100,
			mana: 150,
			HealthRegeneration: 1.2,
			manaRegeneration: 2.0,
			physiqueArmor: 1,
			magicArmor: 1.5,
			scale: 1,
			svg: "flora.svg",
			png: "Flora.jpg",
			glb: "Flora.glb",
		},
		Shadow: {
			name: "Shadow",
			type: "assassin",
			speed: 1.3,
			hitDistance: 6,
			autoAttackDamage: [40, 50, 70, 80, 90],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [40, 50, 70, 80, 90],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [40, 50, 70, 80, 90],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [40, 50, 70, 80, 90],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [40, 50, 70, 80, 90],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 85,
			mana: 90,
			HealthRegeneration: 1.0,
			manaRegeneration: 1.0,
			physiqueArmor: 1,
			magicArmor: 1,
			scale: 1,
			svg: "shadow.svg",
			png: "Shadow.jpg",
			glb: "Shadow.glb",
		},
		Adventurer: {
			name: "Adventurer",
			type: "Lambda",
			speed: 10,
			hitDistance: 10,
			autoAttackDamage: [10, 20, 30, 40, 50],
			autoAttackCd: [2, 1.8, 1.6, 1.4, 1.3],
			skill1Id: 0,
			skill2Id: 1,
			skill3Id: 2,
			ultimatId: 3,
			skill1Damage: [10, 20, 30, 40, 50],
			skill1ManaCost: [10, 15, 30, 45, 60],
			skill1Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill2Damage: [10, 20, 30, 40, 50],
			skill2ManaCost: [10, 15, 30, 45, 60],
			skill2Cd: [2, 1.8, 1.6, 1.4, 1.3],
			skill3Damage: [10, 20, 30, 40, 50],
			skill3ManaCost: [10, 15, 30, 45, 60],
			skill3Cd: [2, 1.8, 1.6, 1.4, 1.3],
			ultimatDamage: [10, 20, 30, 40, 50],
			ultimatManaCost: [10, 15, 30, 45, 60],
			ultimatCd: [2, 1.8, 1.6, 1.4, 1.3],
			health: 100,
			mana: 100,
			HealthRegeneration: 10,
			manaRegeneration: 10,
			physiqueArmor: 10,
			magicArmor: 10,
			scale: 1,
			svg: "Adventurer.svg",
			png: "Adventurer.jpg",
			glb: "Adventurer.glb",
		},
	},
};

module.exports = characters;
